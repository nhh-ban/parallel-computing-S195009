...
MTweedieTests <- function(N, M, sig) {
  ...
  chunk_size <- M / num_chunks
  
  # Instantiate and register the cores
  maxcores <- 8
  ...
  sum_results <- foreach(i = 1:num_chunks, .combine = '+', .packages = c('tweedie'), .export = c('simTweedieTest')) %dopar% {
